...
app.layout = html.Div([
    html.H1("Average Annual and Daily Working Hours per Worker (LSTM)"),
    dcc.Dropdown(
        id='entity-dropdown',
...
def update_graphs(selected_entities):
    ...
    for entity in selected_entities:
        ...
        forecast_annual = forecast_lstm(entity_df, 'Average annual working hours per worker')
    ...
    for entity in selected_entities:
        entity_df = filtered_df[filtered_df['Entity'] == entity]
        forecast_daily = forecast_lstm(entity_df, 'Average daily working hours per worker')
