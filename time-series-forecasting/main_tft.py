...
app.layout = html.Div([
    html.H1("Average Annual and Daily Working Hours per Worker (TFT)"),
    dcc.Dropdown(
        id='entity-dropdown',
...
# Function to preprocess data for TFT
...
# Function to build and train TFT model
def build_and_train_tft(X, y, n_steps=3, n_features=1):
    inputs = Input(shape=(n_steps, n_features))
    attention = MultiHeadAttention(num_heads=2, key_dim=2)(inputs, inputs)
    norm = LayerNormalization()(attention)
    dense1 = Dense(50, activation='relu')(norm)
...
def update_graphs(selected_entities):
    ...
    # Forecasting function using TFT
    def forecast_tft(df, column, periods=100, n_steps=3):
        data = df[[column]].values
        X, y, scaler = preprocess_data(data, n_steps)
        model = build_and_train_tft(X, y, n_steps, n_features=1)
        forecast = []
    ...
    for entity in selected_entities:
        ...
        forecast_annual = forecast_tft(entity_df, 'Average annual working hours per worker')
    ...
    for entity in selected_entities:
        entity_df = filtered_df[filtered_df['Entity'] == entity]
        forecast_daily = forecast_tft(entity_df, 'Average daily working hours per worker')
