...
#from keras.layers import GRU, Dense # github copilot # IDE shows error but code works :|
...
app.layout = html.Div([
    html.H1("Average Annual and Daily Working Hours per Worker (GRU)"),
    dcc.Dropdown(
        id='entity-dropdown',
...
# Function to preprocess data for GRU
...
def build_and_train_gru(X, y, n_steps=3, n_features=1):
    model = models.Sequential()
    model.add(layers.GRU(50, activation='relu', input_shape=(n_steps, n_features)))
...
def update_graphs(selected_entities):
    ...
    # Forecasting function using GRU
    def forecast_gru(df, column, periods=100, n_steps=3):
        data = df[[column]].values
        X, y, scaler = preprocess_data(data, n_steps)
        model = build_and_train_gru(X, y, n_steps, n_features=1)
        forecast = []
    ...
    for entity in selected_entities:
        ...
        forecast_annual = forecast_gru(entity_df, 'Average annual working hours per worker')
    ...
    for entity in selected_entities:
        entity_df = filtered_df[filtered_df['Entity'] == entity]
        forecast_daily = forecast_gru(entity_df, 'Average daily working hours per worker')
